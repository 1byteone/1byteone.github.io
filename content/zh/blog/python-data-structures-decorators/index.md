---
title: "Python 数据结构与装饰器：复用行为而不隐藏边界"
date: 2026-08-21
summary: "从 list、dict 选型到装饰器、生成器和可维护复用。结合真实后端场景拆解 Python Data Structures & Decorators 的边界、失败路径与生产实践。"
tags:
  - Python
  - Data Structures
  - Decorator
...
这张图围绕“An ETL job streams a large CSV, deduplicates IDs with a set, and wraps retries without hiding failures.”展开。学习 Python Data Structures & Decorators 时，不能只记住组件名称：要能说明输入从哪里来、状态由谁持有、哪个组件承担失败、以及如何用指标证明系统仍然健康。白板中的箭头对应代码边界，红色感叹号对应需要显式处理的风险。

## 逐层拆解

主流程是：**Choose the right container; wrap behavior explicitly**。前半段是请求或数据的进入路径，后半段是运行时、存储或执行结果。

- **边界契约**：先确定输入、输出和错误结构；不要让隐式类型、未校验参数或共享可变状态穿透多层。
- **核心机制**：list → ordered mutable sequence；tuple → immutable record-like value。这些机制决定延迟、吞吐和可测试性。
- **资源与状态**：dict → keyed lookup + explicit schema；set → membership / deduplication。生产系统必须说明资源何时创建、何时释放，以及状态是否可恢复。
- **失败路径**：Decorator → wrapper preserves contract；必要时再结合 **Generator → lazy iteration + bounded memory** 做降级、重试或回滚。

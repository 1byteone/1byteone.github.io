...
date: 2026-08-21
summary: "A scenario-driven guide to JVM Memory & Garbage Collection, including boundaries, failure paths, and production trade-offs."
tags:
  - Java
  - JVM
  - GC
...
![JVM Memory & Garbage Collection — Backend Engineering](featured.png)
...
Use this situation as the mental model: **A recommendation API protects p99 latency by measuring allocation bursts rather than guessing GC flags.**. A useful backend diagram answers four questions: where does input enter, who owns state, which boundary can fail, and how do we know the system is healthy? The flow is **Allocation → promotion → reachability → collection**.

## Deconstructing the architecture

- **Contract**: define input, output, and error shape before adding implementation detail.
- **Mechanism**: GC Root → Reachability Analysis; Young Gen: Eden + Survivor From / To. This is the part that determines latency, throughput, and testability.
- **State and resources**: Minor GC → copy live objects; Promotion → long-lived objects enter Old Gen. Explain creation, ownership, cleanup, and recovery.
- **Failure path**: Mark → Sweep / Compact; combine it with **G1 / ZGC → choose for latency goals** when choosing a fallback, retry, or rollback.
